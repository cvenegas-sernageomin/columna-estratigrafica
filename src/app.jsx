...
function freehandUnitPath(x0, xr, topY, hpx, seed) {
  const step = 11;
  const n = Math.max(2, Math.round(hpx / step));
  const A1 = 1.9, A2 = 0.9, f1 = 0.13, f2 = 0.31, ph = seed * 1.7;
                      ...
                      bands.push(
                        fh
                          ? <path key={`${u.id}-a${k}`} fill={`url(#${li[2]})`}
                              d={freehandUnitPath(CX, Math.min(CX + uwA, CX + CW - EDGE_MARGIN), yStart, aH, i + 1)} />
                          : <rect key={`${u.id}-a${k}`} x={CX} y={yStart}
                              width={uwA} height={aH} fill={`url(#${li[2]})`} />,
                        fh
                          ? <path key={`${u.id}-b${k}`} fill={`url(#${li2[2]})`}
                              d={freehandUnitPath(CX, Math.min(CX + uwB, CX + CW - EDGE_MARGIN), yStart + aH, cycH - aH, i + 101)} />
