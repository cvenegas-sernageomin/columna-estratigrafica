              ...
              <svg id="colsvg" width={svgW} height={svgH} xmlns="http://www.w3.org/2000/svg"
                ...
                <g clipPath="url(#colclip)">
                  {units.map((u, i) => {
                    ...
                    const uw   = u.lithoId === "covered" ? CW : CW * gi[2];
                    const hasInter = u.lithoId2 && u.lithoId2 !== u.lithoId;

                    if (!hasInter) {
                      if (edgeStyle === "freehand" && u.lithoId !== "covered") {
                    ...
                    const uwA    = uw;                     // principal (granulometría de la unidad)
                    const uwB    = CW * findG(li2[3])[2];  // secundaria (su grano por defecto)
                    const nCyc   = Math.max(2, Math.min(28, Math.floor(hpx / 30)));
                    const cycH   = hpx / nCyc;
                    const aH     = cycH * ratio;
                    // Bandas delgadas → más sinuosidad (fscale) y menor amplitud,
                    // para que no se vea tosco.
                    const bhA = aH, bhB = cycH - aH;
                      ...
                      bands.push(
                        ...
                        fh
                          ? <path key={`${u.id}-b${k}`} fill={`url(#${li2[2]})`}
                              d={freehandUnitPath(CX, Math.min(CX + uwB, CX + CW - EDGE_MARGIN), yStart + aH, cycH - aH, i + 101, fsB, asB)} />
