...
const BTX = STX + STW + 4;   // pista de bioturbación (BI 0–6)
const BTW = 18;
const ITX = BTX + BTW + 4;   // pista de icnofósiles / trazas
const ITW = 30;
const LX  = ITX + ITW + 6;   // inicio de etiquetas de texto
const SW  = 840;
const PHX = SW - 100;
const PHW = 80, PHH = 60;
...
const LITH = [
  ...
  ["calc_sandstone","Arenisca calcárea",       "p-calcsand","ms",     "Sedimentarias clásticas"],
  ["greywacke",    "Grauvaca",                "p-greyw",   "cs",     "Sedimentarias clásticas"],
  ...
  ["sandy_lime",   "Caliza arenosa",          "p-sandylm", "ms",     "Carbonatos y químicas"],
  ["oolitic_lime", "Caliza oolítica",       "p-oolitlm", "ms",     "Carbonatos y químicas"],
  ["chalk",        "Creta / Chalk",           "p-chalk",   "clay",   "Carbonatos y químicas"],
  ["marl",         "Marga",                   "p-marl",    "silt",   "Carbonatos y químicas"],
  ...
  // ── Tefras — depósitos piroclásticos no litificados ──
  ["tefra_bloques",         "Tefra de bloques",              "p-tefblk",  "cobble",  "Tefras (piroclásticos no litificados)"],
  ["tefra_bombas",          "Tefra de bombas",               "p-tefbmb",  "cobble",  "Tefras (piroclásticos no litificados)"],
  ["tefra_bloques_lapilli", "Tefra de bloques y lapilli",    "p-tefblk",  "cobble",  "Tefras (piroclásticos no litificados)"],
  ["tefra_bombas_lapilli",  "Tefra de bombas y lapilli",     "p-tefbmb",  "cobble",  "Tefras (piroclásticos no litificados)"],
  ["tefra_bloques_ceniza",  "Tefra de bloques y ceniza",     "p-tefblk",  "cobble",  "Tefras (piroclásticos no litificados)"],
  ["tefra_bombas_ceniza",   "Tefra de bombas y ceniza",      "p-tefbmb",  "cobble",  "Tefras (piroclásticos no litificados)"],
  ["tefra_lapilli_bloques", "Tefra de lapilli y bloques",    "p-teflap",  "granule", "Tefras (piroclásticos no litificados)"],
  ["tefra_lapilli_bombas",  "Tefra de lapilli y bombas",     "p-teflap",  "granule", "Tefras (piroclásticos no litificados)"],
  ["tefra_ceniza_bloques",  "Tefra de ceniza y bloques",     "p-tefcen",  "silt",    "Tefras (piroclásticos no litificados)"],
  ["tefra_ceniza_bombas",   "Tefra de ceniza y bombas",      "p-tefcen",  "silt",    "Tefras (piroclásticos no litificados)"],
  ["lapilli_tefra",         "Lapilli",                       "p-teflap",  "granule", "Tefras (piroclásticos no litificados)"],
  ["tefra_lapilli_ceniza",  "Tefra de lapilli y ceniza",     "p-teflap",  "granule", "Tefras (piroclásticos no litificados)"],
  ["tefra_ceniza_lapilli",  "Tefra de ceniza y lapilli",     "p-tefcen",  "silt",    "Tefras (piroclásticos no litificados)"],
  ["ceniza_tefra",          "Ceniza",                        "p-tefcen",  "silt",    "Tefras (piroclásticos no litificados)"],
  ["tefra_indif",           "Tefra indiferenciada",          "p-teflap",  "granule", "Tefras (piroclásticos no litificados)"],
  // ── Rocas volcánicas piroclásticas — litificados ──
  ["brecha_pirocl",     "Brecha piroclástica",                       "p-vbrecc",   "cobble",  "Rocas volcánicas piroclásticas (litificados)"],
  ["aglomerado",        "Aglomerado",                                "p-agglom",   "cobble",  "Rocas volcánicas piroclásticas (litificados)"],
  ["br_tob_blk_lap",    "Brecha tobácea de bloques y lapilli",       "p-vbrecc",   "cobble",  "Rocas volcánicas piroclásticas (litificados)"],
  ["agl_tob_bmb_lap",   "Aglomerado tobáceo de bombas y lapilli",    "p-agglom",   "cobble",  "Rocas volcánicas piroclásticas (litificados)"],
  ["br_tob_blk_cen",    "Brecha tobácea de bloques y ceniza",        "p-vbrecc",   "cobble",  "Rocas volcánicas piroclásticas (litificados)"],
  ["agl_tob_bmb_cen",   "Aglomerado tobáceo de bombas y ceniza",     "p-agglom",   "cobble",  "Rocas volcánicas piroclásticas (litificados)"],
  ["toba_br_lap_blk",   "Toba brechosa de lapilli y bloques",        "p-lapilli",  "granule", "Rocas volcánicas piroclásticas (litificados)"],
  ["toba_agl_lap_bmb",  "Toba aglomerádica de lapilli y bombas",     "p-lapilli",  "granule", "Rocas volcánicas piroclásticas (litificados)"],
  ["toba_br_cen_blk",   "Toba brechosa de ceniza y bloques",         "p-blocktuff","silt",    "Rocas volcánicas piroclásticas (litificados)"],
  ["toba_agl_cen_bmb",  "Toba aglomerádica de ceniza y bombas",      "p-ashtuff",  "silt",    "Rocas volcánicas piroclásticas (litificados)"],
  ["toba_lapilli",       "Toba de lapilli",                           "p-lapilli",  "granule", "Rocas volcánicas piroclásticas (litificados)"],
  ["toba_lap_cen",       "Toba de lapilli y ceniza",                  "p-lapilli",  "granule", "Rocas volcánicas piroclásticas (litificados)"],
  ["toba_cen_lap",       "Toba de ceniza y lapilli",                  "p-ashtuff",  "silt",    "Rocas volcánicas piroclásticas (litificados)"],
  ["toba_ceniza",        "Toba de ceniza",                            "p-ashtuff",  "silt",    "Rocas volcánicas piroclásticas (litificados)"],
  ["roca_pirocl_indif",  "Roca piroclástica indiferenciada",          "p-lapilli",  "granule", "Rocas volcánicas piroclásticas (litificados)"],
  // ── Tufitas / Volcanosedimentarios (25–75% piroclastos) ──
  ["tuffite",          "Tufita",                  "p-tuffite",  "fs",     "Tufitas / Volcanosedimentarios"],
  ["lapillite",        "Lapillita",               "p-lapillite","granule","Tufitas / Volcanosedimentarios"],
  // ── Depósitos epiclásticos (<25% piroclastos) ──
  ["epiclastic",       "Roca epiclástica (retrabajada)","p-epiclas","ms", "Depósitos epiclásticos"],
  ["epiclastic_breccia","Brecha epiclástica",     "p-epibrecc","cobble","Depósitos epiclásticos"],
  // ── Intrusivas plutónicas (serie QAP Streckeisen, félsica → máfica) ──
  ["afs_granite",  "Álcali-feldespato granito","p-afgran", "coherent", "Intrusivas plutónicas"],
  ...
  ["qz_syenite",   "Cuarzosienita",           "p-qsyen",   "coherent", "Intrusivas plutónicas"],
  ["syenite",      "Sienita",                 "p-syen",    "coherent", "Intrusivas plutónicas"],
  ["qz_monzonite", "Cuarzomonzonita",         "p-qmonz",   "coherent", "Intrusivas plutónicas"],
  ["monzonite",    "Monzonita",               "p-monz",    "coherent", "Intrusivas plutónicas"],
  ["monzodiorite", "Monzodiorita",            "p-monzdi",  "coherent", "Intrusivas plutónicas"],
  ["diorite",      "Diorita",                 "p-dior",    "coherent", "Intrusivas plutónicas"],
  ["monzogabbro",  "Monzogabro",              "p-monzgb",  "coherent", "Intrusivas plutónicas"],
  ...
  ["quartzite",    "Cuarcita",                "p-quartzite","coherent","Metamórficas (por textura)"],
  ["marble",       "Mármol",                  "p-marble",  "coherent", "Metamórficas (por textura)"],
  ["hornfels",     "Cornubianita / Hornfels", "p-hornfels","coherent", "Metamórficas (por textura)"],
  ["serpentinite", "Serpentinita",            "p-serpent", "coherent", "Metamórficas (por textura)"],
  // ── Cubierto / sin afloramiento ──
  ["covered",      "Tapado / cubierto (sin afloramiento)", "p-covered", "coherent", "Cubierto / sin afloramiento"],
];

const LITH_GROUPS = [...new Set(LITH.map(l => l[4]))];
// Precomputado: litologías por grupo (LITH es constante; evita filtrar en cada render)
const LITH_BY_GROUP = new Map(LITH_GROUPS.map(g => [g, LITH.filter(l => l[4] === g)]));

// Catálogo de modificadores de zona (techo / muro de una unidad)
// [id, nombre, patternId]
const MOD_TYPES = [
  ["paleosuelo",     "Paleosuelo",               "mod-paleosuelo"],
  ["trazas",         "Trazas fósiles",            "mod-trazas"],
  ["bioturbacion",   "Bioturbación",              "mod-bioturbacion"],
  ["oxidacion",      "Oxidación / hematita",      "mod-oxidacion"],
  ["cemento_calc",   "Cemento calcáreo",          "mod-cemento-calc"],
  ["silicificacion", "Silicificación",            "mod-silice"],
  ["nodulos",        "Nódulos / concreciones",    "mod-nodulos"],
  ["materia_org",    "Materia orgánica / carbón", "mod-carbon"],
];

// Backward-compat aliases for older CSV files
const LITHO_ALIAS = {
  tuff: "toba_ceniza", ash_tuff: "toba_ceniza", lapilli_tuff: "toba_lapilli",
  block_tuff: "brecha_pirocl", ignimbrite: "toba_ceniza",
  agglomerate: "aglomerado", volc_breccia: "brecha_pirocl",
  volcanic_breccia: "brecha_pirocl",
  // retirados en v4.3: la estructura se registra en el campo structSymbols
  xbedded_sand: "sandstone", ripple_sand: "sandstone",
};
...
// Normaliza un lithoId aplicando aliases de versiones anteriores.
// Usar en TODA ruta de entrada de datos (CSV y .col.json).
const normalizeLithoId = id => {
  const k = String(id || "").trim();
  return LITHO_ALIAS[k] || k;
};
const findL = id => LITH.find(l => l[0] === id) ?? LITH[2];
const findG = id => GRAIN.find(g => g[0] === id) ?? GRAIN[4];

// Tabs del panel de edición (orden = orden visual, izquierda a derecha)
const PANEL_TABS = [
  ["meta", "⌖ Ubicación"], ["units", "✚ Unidades"],
  ["thickness", "📐 Espesor"], ["samples", "🧪 Muestras"],
];
...
// ═══════════════════════════════════════════════════════════════
// ICHNOFOSSILS — trazas fósiles e icnofacies  [id, label, group]
// Tucker (2003) §5.6; icnofacies de Seilacher
// ═══════════════════════════════════════════════════════════════
const ICHNO_TYPES = [
  // ── Ichnofacies Skolithos (alta energía, somero-intermareale) ──
  ["skolithos",      "Skolithos",           "Skolithos"],
  ["ophiomorpha",    "Ophiomorpha",         "Skolithos"],
  ["arenicolites",   "Arenicolites",        "Skolithos"],
  ["diplocraterion", "Diplocraterion",      "Skolithos"],
  // ── Ichnofacies Cruziana (moderada, shoreface–plataforma) ──
  ["cruziana",       "Cruziana",            "Cruziana"],
  ["rusophycus",     "Rusophycus",          "Cruziana"],
  ["rhizocorallium", "Rhizocorallium",      "Cruziana"],
  ["asteriacites",   "Asteriacites",        "Cruziana"],
  // ── Ichnofacies Zoophycos (baja energía, plataforma–talud) ──
  ["zoophycos",      "Zoophycos",           "Zoophycos"],
  ["chondrites",     "Chondrites",          "Zoophycos"],
  ["planolites",     "Planolites",          "Zoophycos"],
  // ── Ichnofacies Nereites (cuenca profunda) ──
  ["nereites",       "Nereites",            "Nereites"],
  ["paleodictyon",   "Paleodictyon",        "Nereites"],
  ["cosmorhaphe",    "Cosmorhaphe",         "Nereites"],
  // ── Otros icnofósiles comunes ──
  ["thalassinoides", "Thalassinoides",      "Otros"],
  ["terebellina",    "Terebellina",         "Otros"],
];

const ICHNO_GROUPS = [...new Set(ICHNO_TYPES.map(t => t[2]))];
const findI = id => ICHNO_TYPES.find(t => t[0] === id);

function ichnoGlyphEls(id, k = "ig") {
  ...
  switch (id) {
    case "skolithos":      return [ln(0,-10,0,10,2)];
    case "ophiomorpha":    return [pa("M0,-10 L0,4 Q0,10 -5,10",2),pa("M-8,2 L0,2",0.9),pa("M-8,6 L-2,6",0.9)];
    case "arenicolites":   return [pa("M-7,-8 L-7,5 Q-7,10 0,10 Q7,10 7,5 L7,-8")];
    case "diplocraterion": return [pa("M-7,-8 L-7,5 Q-7,10 0,10 Q7,10 7,5 L7,-8"),ln(-6,0,6,0,0.7),ln(-5,-3,5,-3,0.7),ln(-4,-6,4,-6,0.7)];
    case "cruziana":       return [pa("M-10,0 C-8,-6 8,-6 10,0 C8,6 -8,6 -10,0 Z"),pa("M-7,-2 L-3,0 L-7,2",0.7),pa("M0,-2 L4,0 L0,2",0.7),pa("M7,-2 L10,0 L7,2",0.7)];
    case "rusophycus":     return [pa("M-8,-4 Q0,-8 8,-4 L8,4 Q0,8 -8,4 Z"),pa("M-4,-1 L4,-1",0.7),pa("M-4,2 L4,2",0.7)];
    case "rhizocorallium": return [pa("M-11,-3 L11,-3"),pa("M-11,4 L11,4"),pa("M-11,-3 Q-11,0.5 -11,4"),pa("M11,-3 Q11,0.5 11,4"),ln(-7,-1,7,-1,0.6),ln(-6,1,6,1,0.6)];
    case "asteriacites":   return [pa("M0,-10 L0,0"),pa("M0,0 L9.5,3"),pa("M0,0 L5.9,8"),pa("M0,0 L-5.9,8"),pa("M0,0 L-9.5,3")];
    case "zoophycos":      return [pa("M0,0 C3,-6 9,-4 9,0 C9,6 3,9 0,5 C-4,1 -3,-9 3,-9 C9,-9 11,-2 8,4")];
    case "chondrites":     return [pa("M0,9 L0,2"),pa("M0,2 L-5,-3"),pa("M0,2 L5,-3"),pa("M-5,-3 L-8,-8",0.9),pa("M-5,-3 L-2,-8",0.9),pa("M5,-3 L2,-8",0.9),pa("M5,-3 L8,-8",0.9)];
    case "planolites":     return [pa("M-10,0 Q-5,6 0,0 Q5,-6 10,0")];
    case "nereites":       return [pa("M-10,4 Q-7,-3 -4,4 Q-1,-3 2,4 Q5,-3 8,4"),ci(-7,0.5,1.4),ci(-1,0.5,1.4),ci(5,0.5,1.4)];
    case "paleodictyon":   return [pa("M-9,-5 L0,-10 L9,-5 L9,5 L0,10 L-9,5 Z"),pa("M-9,-5 L9,5"),pa("M9,-5 L-9,5"),pa("M0,-10 L0,10")];
    case "cosmorhaphe":    return [pa("M-10,0 C-8,-7 -2,-7 0,0 C2,7 8,7 10,0 C8,-7 2,-7 0,0 C-2,7 -8,7 -10,0")];
    case "thalassinoides": return [pa("M0,-9 L0,0"),pa("M0,0 L-7,7"),pa("M0,0 L7,7"),ci(0,-9,1.5),ci(-7,7,1.5),ci(7,7,1.5)];
    case "terebellina":    return [pa("M0,-10 C6,-10 10,-5 10,0 C10,6 5,10 0,10 C-5,10 -10,6 -10,0 C-10,-5 -6,-10 0,-10"),ln(-10,0,10,0,0.6)];
    default:               return [ci(0,0,2)];
  }
}

function IchnoIcon({ id, size = 22 }) {
  return (
...
function glyphEls(id, k = "g") {
  ...
  switch (id) {
    ...
    case "graded_normal":  return [pa("M-8,8 L8,8 L0,-8 Z")];   // base gruesa abajo, vértice arriba
    case "graded_reverse": return [pa("M0,8 L8,-8 L-8,-8 Z")]; // base gruesa arriba, vértice abajo
...
// Margen reservado a la derecha para que el borde sinuoso quepa incluso cuando
// la granulometría es máxima (ancho completo).
const EDGE_MARGIN = 5;

// Borde derecho "a mano alzada" (baja sinuosidad): camino cerrado de la unidad
// con la arista derecha ondulada suavemente alrededor de xr. Determinista por seed.
function freehandUnitPath(x0, xr, topY, hpx, seed, fscale = 1, ampScale = 1) {
  const A1 = 1.9 * ampScale, A2 = 0.9 * ampScale;
  const f1 = 0.13 * fscale, f2 = 0.31 * fscale, ph = seed * 1.7;
  const ox = y => A1 * Math.sin(y * f1 + ph) + A2 * Math.sin(y * f2 + ph * 1.3);
  // muestreo más denso cuando hay más sinuosidad (fscale alto) → curva suave
  const step = Math.max(2.5, Math.min(11, 9 / fscale));
  const n = Math.max(2, Math.round(hpx / step));
  let d = `M${x0},${topY.toFixed(1)}`;
  for (let i = 0; i <= n; i++) {
    const y = topY + hpx * i / n;
    d += ` L${(xr + ox(y)).toFixed(1)},${y.toFixed(1)}`;
  }
  d += ` L${x0},${(topY + hpx).toFixed(1)} Z`;
  return d;
}
...
// CONTACT GEOMETRY
// ═══════════════════════════════════════════════════════════════
// Gradacional: línea recta punteada (dasharray en render)
// Erosivo: línea irregular ondulada nítida
const ERO_DY = [-4, 5, -5, 3, -6, 4, -5, 5, -4, 6];
function cPath(type, x1, x2, y) {
  if (type === "gradational") return `M${x1},${y}L${x2},${y}`;
  if (type === "erosive") {
    const n = 10, s = (x2 - x1) / n; let d = `M${x1},${y}`;
    for (let i = 0; i < n; i++)
      d += ` C${x1+i*s+s*.3},${y+ERO_DY[i]} ${x1+i*s+s*.7},${y+ERO_DY[i]} ${x1+(i+1)*s},${y}`;
    return d;
  }
  return `M${x1},${y}L${x2},${y}`;
}
const cClr = t => t==="erosive" ? T.rust : T.ink;
const cWid = t => t==="erosive" ? 2 : 1.3;
const cDash = t => t==="gradational" ? "5,4" : "none";
...
const CSV_HEADERS = ["thickness","lithology","lithology2","lithoRatio","color","grainSize","structures",
  ...
  "thicknessMode","interpLi1MinCm","interpLi1MaxCm","interpLi2MinCm","interpLi2MaxCm","beddingTrend",
  "modTechoType","modTechoCm","modTechoNotes","modMuroType","modMuroCm","modMuroNotes",
  "bioturbIndex","traceIchnos"];

// Bloque de referencia (líneas "#", ignoradas al importar) con todos los códigos
// de litología aceptados en "lithology" / "lithology2", agrupados como en el picker.
const LITH_REFERENCE_BLOCK = "# ── CÓDIGOS DE LITOLOGÍA VÁLIDOS para lithology / lithology2 (usar el código, no el nombre) ──\n"
  + LITH_GROUPS.map(g => `# ${g}:\n`
      + LITH_BY_GROUP.get(g).map(l => `#   ${l[0]} = ${l[1]}`).join("\n")
    ).join("\n");
...
20.00,toba_lapilli,lapillite,0.6,"5YR 5/3 marrón rojizo",granule,"Intercalación rítmica, gradaciones normales por evento",,sharp,Volcánico subaéreo,,fresco,Paquete intercalado toba-lapillita (60/40)
0.90,toba_ceniza,,,"N7 gris claro",silt,"Laminación fina, masiva",,gradational,Volcánico subaéreo,,fresco,Toba de ceniza fina
4.10,toba_ceniza,,,"5YR 5/2 marrón rojizo pálido",fs,"Fiamme, soldamiento, juntas de enfriamiento",,erosive,Volcánico subaéreo,,ligeramente alterado,Ignimbrita soldada — flujo piroclástico
...
1.20,block_tuff,,,"5YR 4/3 marrón rojizo",cobble,"Bloques y bombas, soporte matriz",,erosive,Volcánico subaéreo,,moderadamente alterado,Depósito proximal
${LITH_REFERENCE_BLOCK}
`;

const META_KEYS = ["project","locality","author","date","datum","utmZone","utmHemi",
  "utmE","utmN","utmE2","utmN2","elevation","azimuth","description"];
...
function generateCsv(units, meta, samples) {
  ...
  const rows = units.map(u => CSV_HEADERS.map(h => {
    const map = {
      ...
      thicknessMode: u.thicknessMode,
      interpLi1MinCm: u.interpLi1MinCm, interpLi1MaxCm: u.interpLi1MaxCm,
      interpLi2MinCm: u.interpLi2MinCm, interpLi2MaxCm: u.interpLi2MaxCm,
      beddingTrend: u.beddingTrend || "",
      modTechoType:  u.modTecho?.type  || "", modTechoCm:    u.modTecho?.cm    || "",
      modTechoNotes: u.modTecho?.notes || "", modMuroType:   u.modMuro?.type   || "",
      modMuroCm:     u.modMuro?.cm     || "", modMuroNotes:  u.modMuro?.notes  || "",
      bioturbIndex:  u.bioturbIndex != null ? u.bioturbIndex : "",
      traceIchnos:   (u.traceIchnos || []).join(";"),
    };
    return esc(map[h]);
  }).join(","));
  ...
  if (Array.isArray(samples)) samples
    .filter(s => s && s.code)
    .forEach(s => head.push(`#sample=${s.height}|${String(s.code).replace(/[|\r\n]/g, " ")}|${s.type || "otra"}|${s.utmE || ""}|${s.utmN || ""}`));
  return head.length ? head.join("\n") + "\n" + body : body;
}
const VALID_MOD_IDS = new Set(MOD_TYPES.map(m => m[0]));
function parseMod(type, cm, notes, unitThicknessM) {
  if (!type || !VALID_MOD_IDS.has(type)) return null;
  const maxCm = Math.floor(unitThicknessM * 100);
  const cmVal = Math.min(Math.max(1, parseFloat(cm) || 1), maxCm);
  return { type, cm: cmVal, notes: (notes || "").slice(0, 120) };
}
...
function parseCsvSamples(text) {
  ...
  text.replace(/^﻿/, "").split(/\r?\n/).forEach(line => {
    ...
    if (!mt) return;
    const [h, code, type, utmE, utmN] = mt[1].split("|");
    if (code && code.trim()) {
      const s = { height: parseFloat(h) || 0, code: code.trim(),
        type: valid.has((type || "").trim()) ? type.trim() : "otra" };
      if ((utmE || "").trim() && (utmN || "").trim()) { s.utmE = utmE.trim(); s.utmN = utmN.trim(); }
      out.push(s);
    }
  });
  return out;
...
// Azimut (grados, 0-360 desde el N de cuadrícula) del recorrido (E1,N1) -> (E2,N2)
...
function buildKml(meta, units, start, end, az, hasImg) {
  ...
  const metaRows = [
    ...
    ["UTM base", `${meta.utmZone||""}${meta.utmHemi||""}  E ${meta.utmE||""} / N ${meta.utmN||""}`],
    end ? ["UTM techo", `E ${meta.utmE2||""} / N ${meta.utmN2||""}`] : null,
    ["Azimut sección", (az != null && isFinite(az)) ? Math.round(az) + "°" : meta.azimuth],
    ["Cota base (m s.n.m.)", meta.elevation],
...
<tr><th>Tramo (m)</th><th>Litología</th><th>Ambiente</th></tr>${rows}</table>
  ...
  const heading = (az != null && isFinite(az)) ? az
    : (isFinite(parseFloat(meta.azimuth)) ? parseFloat(meta.azimuth) : 0);
  // Cámara en la base mirando hacia el techo: heading = azimut del recorrido,
  // tilt alto (casi horizontal) para "mirar hacia el techo" en vez de picado.
  const lookAt = `<LookAt><longitude>${start.lon.toFixed(8)}</longitude>`
    + `<latitude>${start.lat.toFixed(8)}</latitude><altitude>${ele}</altitude>`
    + `<heading>${heading.toFixed(1)}</heading><tilt>78</tilt><range>550</range>`
    + `<altitudeMode>relativeToGround</altitudeMode></LookAt>`;
  const azTxt = (az != null && isFinite(az)) ? Math.round(az) + "°" : (meta.azimuth || "");
  let extra = "";
  if (end) {
    extra = `\n<Placemark><name>${name} (techo)</name>`
      + `<Style><IconStyle><color>ff00aaff</color></IconStyle></Style>`
      + `<Point><coordinates>${end.lon.toFixed(8)},${end.lat.toFixed(8)},0</coordinates></Point></Placemark>`
      + `\n<Placemark><name>Traza de sección${azTxt ? " (az " + azTxt + ")" : ""}</name>`
      + `<Style><LineStyle><color>ff0000ff</color><width>3</width></LineStyle></Style>`
      + `<LineString><tessellate>1</tessellate><coordinates>`
      + `${start.lon.toFixed(8)},${start.lat.toFixed(8)},0 ${end.lon.toFixed(8)},${end.lat.toFixed(8)},0`
      + `</coordinates></LineString></Placemark>`;
  }
  // Tour: garantiza el vuelo a la base (con el azimut) + abrir el globo, con un
  // clic en "Reproducir". El LookAt a nivel Document no siempre auto-vuela.
  const tour = `\n<gx:Tour><name>▶ Volar a la base (vista azimut)</name><gx:Playlist>`
    + `<gx:FlyTo><gx:duration>2.5</gx:duration><gx:flyToMode>bounce</gx:flyToMode>${lookAt}</gx:FlyTo>`
    + `<gx:AnimatedUpdate><Update><targetHref></targetHref><Change>`
    + `<Placemark targetId="basePM"><gx:balloonVisibility>1</gx:balloonVisibility></Placemark>`
    + `</Change></Update></gx:AnimatedUpdate>`
    + `</gx:Playlist></gx:Tour>`;
  // Vista "overlay": la columna fijada en pantalla (toggle aparte, apagada por defecto).
  const overlay = hasImg
    ? `\n<ScreenOverlay><name>Columna (overlay en pantalla)</name><visibility>0</visibility>`
      + `<Icon><href>files/column.png</href></Icon>`
      + `<overlayXY x="1" y="1" xunits="fraction" yunits="fraction"/>`
      + `<screenXY x="0.99" y="0.97" xunits="fraction" yunits="fraction"/>`
      + `<size x="0" y="0.92" xunits="fraction" yunits="fraction"/></ScreenOverlay>`
    : "";
...
function SvgDefs({ totalH, spx }) {
  ...
  return (
    <defs>
      ...
      <clipPath id="biclip">
        <rect x={BTX} y={MT} width={BTW} height={totalH * SPX} />
      </clipPath>
      <clipPath id="ichnoclip">
        <rect x={ITX} y={MT} width={ITW} height={totalH * SPX} />
      </clipPath>
      ...
      {/* FGDC 613 — Calcareous sandstone: stipple + brick elements */}
      <pattern id="p-calcsand" width="18" height="12" patternUnits="userSpaceOnUse">
        <rect width="18" height="12" fill="#E8D490"/>
        <circle cx="3" cy="3" r=".7" fill="#A08020"/><circle cx="10" cy="2" r=".6" fill="#A08020"/>
        <circle cx="6" cy="7" r=".7" fill="#A08020"/><circle cx="14" cy="8" r=".6" fill="#A08020"/>
        <circle cx="16" cy="4" r=".5" fill="#A08020"/><circle cx="1" cy="10" r=".5" fill="#A08020"/>
        <rect x="7" y="9" width="7" height="3" fill="none" stroke="#7A6840" strokeWidth=".6"/>
      </pattern>
      {/* FGDC 654 — Greywacke/subgraywacke: coarse stipple, dark */}
      <pattern id="p-greyw" width="12" height="10" patternUnits="userSpaceOnUse">
        <rect width="12" height="10" fill="#A09478"/>
        <circle cx="3" cy="3" r="1.1" fill="#786844"/><circle cx="9" cy="7" r="1.2" fill="#786844"/>
        <circle cx="7" cy="2" r=".8" fill="#8A7A56"/><circle cx="1" cy="8" r=".9" fill="#8A7A56"/>
        <circle cx="11" cy="3" r=".7" fill="#8A7A56"/>
      </pattern>
      ...
      <pattern id="p-dolo" width="18" height="12" patternUnits="userSpaceOnUse">
        <rect width="18" height="12" fill="#D8C8B0"/>
        <rect x=".5" y=".5" width="15" height="5" fill="none" stroke="#7A5E36" strokeWidth=".7"/>
        <rect x="9" y="6" width="15" height="5" fill="none" stroke="#7A5E36" strokeWidth=".7"/>
        <line x1="2" y1="0" x2="6" y2="6" stroke="#7A5E36" strokeWidth=".6"/>
        <line x1="10" y1="6" x2="14" y2="12" stroke="#7A5E36" strokeWidth=".6"/>
      </pattern>
      <pattern id="p-chert" width="12" height="10" patternUnits="userSpaceOnUse">
        <rect width="12" height="10" fill="#909090"/>
        <line x1="0" y1="5" x2="12" y2="5" stroke="#4A4A4A" strokeWidth=".7"/>
        <path d="M3,3 l1.5,-2.5 l1.5,2.5" fill="none" stroke="#4A4A4A" strokeWidth=".8"/>
        <path d="M9,8 l1.5,-2.5 l1.5,2.5" fill="none" stroke="#4A4A4A" strokeWidth=".8"/>
      </pattern>
      ...
      {/* FGDC 636 — Sandy limestone: brick + stipple dots */}
      <pattern id="p-sandylm" width="18" height="12" patternUnits="userSpaceOnUse">
        <rect width="18" height="12" fill="#B8C8D0"/>
        <rect x=".5" y=".5" width="15" height="5" fill="none" stroke="#356296" strokeWidth=".6"/>
        <rect x="9" y="6" width="15" height="5" fill="none" stroke="#356296" strokeWidth=".6"/>
        <circle cx="4" cy="3" r=".7" fill="#8A7820"/><circle cx="12" cy="8" r=".7" fill="#8A7820"/>
        <circle cx="14" cy="3" r=".6" fill="#8A7820"/>
      </pattern>
      {/* FGDC 635 — Oolitic limestone: brick + small circles */}
      <pattern id="p-oolitlm" width="18" height="12" patternUnits="userSpaceOnUse">
        <rect width="18" height="12" fill="#BCC8D4"/>
        <rect x=".5" y=".5" width="15" height="5" fill="none" stroke="#356296" strokeWidth=".6"/>
        <rect x="9" y="6" width="15" height="5" fill="none" stroke="#356296" strokeWidth=".6"/>
        <circle cx="5" cy="3" r="1.4" fill="none" stroke="#356296" strokeWidth=".5"/>
        <circle cx="13" cy="9" r="1.4" fill="none" stroke="#356296" strokeWidth=".5"/>
        <circle cx="15" cy="3" r="1.1" fill="none" stroke="#356296" strokeWidth=".5"/>
      </pattern>
      {/* FGDC 626 — Chalk: very fine horizontal lines + stipple */}
      <pattern id="p-chalk" width="10" height="8" patternUnits="userSpaceOnUse">
        <rect width="10" height="8" fill="#E8E4DC"/>
        <line x1="0" y1="4" x2="10" y2="4" stroke="#B0A89C" strokeWidth=".5"/>
        <circle cx="2" cy="2" r=".4" fill="#C0B8A8"/><circle cx="7" cy="6" r=".4" fill="#C0B8A8"/>
        <circle cx="5" cy="1" r=".35" fill="#C0B8A8"/><circle cx="9" cy="3" r=".35" fill="#C0B8A8"/>
      </pattern>
      {/* FGDC 623 — Marl / calcareous shale: horizontal lines + brick hints */}
      <pattern id="p-marl" width="18" height="10" patternUnits="userSpaceOnUse">
        <rect width="18" height="10" fill="#B0B8A8"/>
        <line x1="0" y1="3" x2="18" y2="3" stroke="#5A6E56" strokeWidth=".7"/>
        <line x1="0" y1="7" x2="18" y2="7" stroke="#5A6E56" strokeWidth=".7"/>
        <rect x="4" y="3.5" width="8" height="3" fill="none" stroke="#356296" strokeWidth=".5"/>
      </pattern>
      ...
      {/* ── Tefras — no litificados (fondo claro, fragmentos sueltos) ── */}
      <pattern id="p-tefblk" width="28" height="24" patternUnits="userSpaceOnUse">
        <rect width="28" height="24" fill="#E8D8C0"/>
        <polygon points="3,3 10,2 13,9 6,13 1,8" fill="none" stroke="#8A7A5E" strokeWidth="1" strokeDasharray="2,1"/>
        <polygon points="16,10 24,9 26,18 19,21 14,16" fill="none" stroke="#8A7A5E" strokeWidth="1" strokeDasharray="2,1"/>
        <polygon points="15,1 20,2 21,6 16,7" fill="none" stroke="#8A7A5E" strokeWidth=".8" strokeDasharray="2,1"/>
        <circle cx="7" cy="19" r=".8" fill="#A8986E"/>
        <circle cx="24" cy="4" r=".7" fill="#A8986E"/>
      </pattern>
      <pattern id="p-tefbmb" width="28" height="24" patternUnits="userSpaceOnUse">
        <rect width="28" height="24" fill="#E8D8C0"/>
        <ellipse cx="8" cy="7" rx="6" ry="4.5" fill="none" stroke="#8A7A5E" strokeWidth="1" strokeDasharray="2,1"/>
        <ellipse cx="21" cy="17" rx="5.5" ry="4" fill="none" stroke="#8A7A5E" strokeWidth="1" strokeDasharray="2,1"/>
        <ellipse cx="22" cy="4" rx="3" ry="2.2" fill="none" stroke="#8A7A5E" strokeWidth=".8" strokeDasharray="2,1"/>
        <circle cx="5" cy="19" r=".8" fill="#A8986E"/>
      </pattern>
      <pattern id="p-teflap" width="18" height="16" patternUnits="userSpaceOnUse">
        <rect width="18" height="16" fill="#E2D4B8"/>
        <circle cx="4" cy="4" r="2.2" fill="none" stroke="#8A7A5E" strokeWidth=".8" strokeDasharray="1.5,1"/>
        <circle cx="12" cy="5" r="2" fill="none" stroke="#8A7A5E" strokeWidth=".7" strokeDasharray="1.5,1"/>
        <circle cx="8" cy="11" r="2.3" fill="none" stroke="#8A7A5E" strokeWidth=".8" strokeDasharray="1.5,1"/>
        <circle cx="15" cy="13" r="1.8" fill="none" stroke="#8A7A5E" strokeWidth=".7" strokeDasharray="1.5,1"/>
        <circle cx="2" cy="14" r="1.5" fill="none" stroke="#8A7A5E" strokeWidth=".7" strokeDasharray="1.5,1"/>
      </pattern>
      <pattern id="p-tefcen" width="10" height="10" patternUnits="userSpaceOnUse">
        <rect width="10" height="10" fill="#EDE4D0"/>
        <circle cx="2" cy="2" r=".5" fill="#B8A880"/>
        <circle cx="7" cy="3" r=".4" fill="#B8A880"/>
        <circle cx="4" cy="6" r=".5" fill="#B8A880"/>
        <circle cx="9" cy="7" r=".4" fill="#B8A880"/>
        <circle cx="1" cy="9" r=".4" fill="#B8A880"/>
        <circle cx="6" cy="9" r=".5" fill="#B8A880"/>
      </pattern>
      ...
      {/* ── Intrusivas plutónicas (serie QAP, félsica → máfica) ── */}
      {/* Álcali-feldespato granito: muy félsico, FK rosado dominante + cuarzo */}
      <pattern id="p-afgran" width="14" height="14" patternUnits="userSpaceOnUse">
        <rect width="14" height="14" fill="#E4CEC6"/>
        <ellipse cx="3" cy="4" rx="1.7" ry="1.1" fill="#D8A890" transform="rotate(30 3 4)"/>
        <ellipse cx="10" cy="8" rx="1.6" ry="1" fill="#D29E86" transform="rotate(-40 10 8)"/>
        <ellipse cx="6" cy="11" rx="1.3" ry=".9" fill="#9C9C98" transform="rotate(15 6 11)"/>
        <circle cx="11" cy="3" r=".7" fill="#4A4040"/>
      </pattern>
      ...
      {/* Sienita: FK rosado dominante, sin cuarzo, anfíbol oscuro */}
      <pattern id="p-syen" width="14" height="14" patternUnits="userSpaceOnUse">
        <rect width="14" height="14" fill="#DDC6C0"/>
        <ellipse cx="3" cy="4" rx="1.8" ry="1.1" fill="#D2A28E" transform="rotate(30 3 4)"/>
        <ellipse cx="10" cy="7" rx="1.7" ry="1.1" fill="#C89A86" transform="rotate(-45 10 7)"/>
        <ellipse cx="5" cy="11" rx="1.5" ry="1" fill="#CEA092" transform="rotate(20 5 11)"/>
        <ellipse cx="12" cy="12" rx="1.1" ry=".7" fill="#3A3632" transform="rotate(10 12 12)"/>
        <circle cx="11" cy="3" r=".7" fill="#2E2A28"/>
      </pattern>
      {/* Cuarzosienita: sienita + algo de cuarzo gris */}
      <pattern id="p-qsyen" width="14" height="14" patternUnits="userSpaceOnUse">
        <rect width="14" height="14" fill="#DECAC4"/>
        <ellipse cx="3" cy="4" rx="1.7" ry="1.1" fill="#D2A28E" transform="rotate(30 3 4)"/>
        <ellipse cx="10" cy="7" rx="1.5" ry="1" fill="#CA9C88" transform="rotate(-45 10 7)"/>
        <ellipse cx="6" cy="11" rx="1.4" ry=".9" fill="#9C9C98" transform="rotate(20 6 11)"/>
        <ellipse cx="2" cy="11" rx="1.1" ry=".8" fill="#8E8E8A" transform="rotate(-10 2 11)"/>
        <circle cx="12" cy="3" r=".7" fill="#332E2C"/>
      </pattern>
      {/* Monzonita: FK rosado + plagioclasa blanca en igual proporción, sin cuarzo */}
      <pattern id="p-monz" width="14" height="14" patternUnits="userSpaceOnUse">
        <rect width="14" height="14" fill="#CFC6C0"/>
        <ellipse cx="3" cy="4" rx="1.6" ry="1" fill="#CC9E8A" transform="rotate(30 3 4)"/>
        <ellipse cx="10" cy="7" rx="1.6" ry="1" fill="#E2E0DA" transform="rotate(-45 10 7)"/>
        <ellipse cx="5" cy="11" rx="1.4" ry=".9" fill="#C89A86" transform="rotate(20 5 11)"/>
        <ellipse cx="11" cy="11" rx="1.2" ry=".8" fill="#DEDCD6" transform="rotate(-10 11 11)"/>
        <circle cx="12" cy="3" r=".8" fill="#322C2A"/>
        <circle cx="6" cy="7" r=".7" fill="#3A3430"/>
      </pattern>
      {/* Cuarzomonzonita: monzonita + cuarzo gris (clásica de pórfidos) */}
      <pattern id="p-qmonz" width="14" height="14" patternUnits="userSpaceOnUse">
        <rect width="14" height="14" fill="#D0C6C0"/>
        <ellipse cx="3" cy="4" rx="1.6" ry="1" fill="#CC9E8A" transform="rotate(30 3 4)"/>
        <ellipse cx="10" cy="7" rx="1.5" ry="1" fill="#E0DED8" transform="rotate(-45 10 7)"/>
        <ellipse cx="6" cy="11" rx="1.3" ry=".9" fill="#9A9A96" transform="rotate(20 6 11)"/>
        <ellipse cx="2" cy="10" rx="1.1" ry=".8" fill="#C89A86" transform="rotate(-10 2 10)"/>
        <circle cx="12" cy="3" r=".8" fill="#322C2A"/>
        <circle cx="9" cy="12" r=".7" fill="#3A3430"/>
      </pattern>
      {/* Monzodiorita: más máfica, plagioclasa gris + máficos abundantes, tinte rosado tenue */}
      <pattern id="p-monzdi" width="13" height="13" patternUnits="userSpaceOnUse">
        <rect width="13" height="13" fill="#BFC0B8"/>
        <ellipse cx="3" cy="4" rx="1.5" ry="1" fill="#C49888" transform="rotate(30 3 4)"/>
        <circle cx="9" cy="7" r="1.3" fill="#2E2A28"/>
        <circle cx="5" cy="11" r="1.1" fill="#3A3632"/>
        <ellipse cx="11" cy="11" rx="1.2" ry=".8" fill="#D8D6CE" transform="rotate(-15 11 11)"/>
        <circle cx="11" cy="3" r="1" fill="#262220"/>
      </pattern>
      ...
      {/* Monzogabro: máfico oscuro, plagioclasa clara + piroxeno + FK rosado tenue */}
      <pattern id="p-monzgb" width="16" height="16" patternUnits="userSpaceOnUse">
        <rect width="16" height="16" fill="#565A52"/>
        <rect x="2" y="3" width="4" height="1.6" fill="#C6CABF" transform="rotate(25 4 3.8)"/>
        <rect x="9" y="6" width="4" height="1.5" fill="#C0A89C" transform="rotate(-40 11 6.8)"/>
        <rect x="4" y="11" width="3.6" height="1.4" fill="#BEC2BA" transform="rotate(15 5.8 11.7)"/>
        <circle cx="12" cy="3" r="1.4" fill="#2A2E2A"/>
        <circle cx="7" cy="8" r="1.2" fill="#24281F"/>
        <circle cx="13" cy="13" r="1.1" fill="#2A2E2A"/>
      </pattern>
      ...
      {/* FGDC 702 — Quartzite: random short dashes at angles */}
      <pattern id="p-quartzite" width="14" height="14" patternUnits="userSpaceOnUse">
        <rect width="14" height="14" fill="#CCCCC4"/>
        <line x1="2" y1="3" x2="5" y2="2" stroke="#74746C" strokeWidth="1"/>
        <line x1="8" y1="6" x2="11" y2="8" stroke="#74746C" strokeWidth="1"/>
        <line x1="3" y1="10" x2="6" y2="12" stroke="#74746C" strokeWidth="1"/>
        <line x1="10" y1="2" x2="12" y2="4" stroke="#74746C" strokeWidth=".8"/>
        <line x1="6" y1="7" x2="8" y2="5" stroke="#74746C" strokeWidth=".8"/>
        <line x1="11" y1="12" x2="14" y2="11" stroke="#74746C" strokeWidth=".8"/>
      </pattern>
      {/* Marble: recrystallized brick + mosaic */}
      <pattern id="p-marble" width="16" height="14" patternUnits="userSpaceOnUse">
        <rect width="16" height="14" fill="#D8D8D0"/>
        <path d="M0,5 Q4,3 8,5 Q12,7 16,5" fill="none" stroke="#9898A0" strokeWidth=".6"/>
        <path d="M0,10 Q4,8 8,10 Q12,12 16,10" fill="none" stroke="#9898A0" strokeWidth=".6"/>
        <polygon points="3,2 6,1 7,4 4,5" fill="none" stroke="#A8A8B0" strokeWidth=".4"/>
        <polygon points="10,7 13,6 14,9 11,10" fill="none" stroke="#A8A8B0" strokeWidth=".4"/>
        <polygon points="5,9 8,8 9,11 6,12" fill="none" stroke="#A8A8B0" strokeWidth=".4"/>
      </pattern>
      {/* Hornfels / cornubianita: dense speckled, dark */}
      <pattern id="p-hornfels" width="10" height="10" patternUnits="userSpaceOnUse">
        <rect width="10" height="10" fill="#686860"/>
        <circle cx="2" cy="2" r=".8" fill="#4A4A44"/><circle cx="7" cy="3" r=".9" fill="#4A4A44"/>
        <circle cx="4" cy="7" r=".9" fill="#4A4A44"/><circle cx="9" cy="8" r=".8" fill="#4A4A44"/>
        <circle cx="1" cy="5" r=".6" fill="#565650"/><circle cx="6" cy="9" r=".6" fill="#565650"/>
        <rect x="7" y="5" width="2" height=".8" fill="#888880" transform="rotate(30 8 5.4)"/>
      </pattern>
      {/* FGDC 710 — Serpentinite: wavy serpentine pattern */}
      <pattern id="p-serpent" width="16" height="12" patternUnits="userSpaceOnUse">
        <rect width="16" height="12" fill="#4A6050"/>
        <path d="M0,3 Q4,1 8,3 Q12,5 16,3" fill="none" stroke="#2E3E34" strokeWidth=".9"/>
        <path d="M0,7 Q4,5 8,7 Q12,9 16,7" fill="none" stroke="#2E3E34" strokeWidth=".9"/>
        <path d="M0,11 Q4,9 8,11 Q12,13 16,11" fill="none" stroke="#2E3E34" strokeWidth=".9"/>
        <ellipse cx="5" cy="5" rx="1.8" ry=".8" fill="#3A5244" transform="rotate(-10 5 5)"/>
        <ellipse cx="12" cy="9" rx="1.6" ry=".7" fill="#3A5244" transform="rotate(10 12 9)"/>
      </pattern>

      {/* ── Cubierto / sin afloramiento ── */}
      <pattern id="p-covered" width="10" height="10" patternUnits="userSpaceOnUse">
        <rect width="10" height="10" fill="#E6E1D6"/>
        <line x1="-5" y1="5" x2="5" y2="-5" stroke="#9C9080" strokeWidth="1.1"/>
        <line x1="0" y1="10" x2="10" y2="0" stroke="#9C9080" strokeWidth="1.1"/>
        <line x1="5" y1="15" x2="15" y2="5" stroke="#9C9080" strokeWidth="1.1"/>
      </pattern>

      {/* ── Modificadores de zona (overlay semitransparente sobre litología) ── */}
      <pattern id="mod-paleosuelo" width="10" height="8" patternUnits="userSpaceOnUse">
        <circle cx="2.5" cy="3" r="1.4" fill="#6B4D10"/>
        <circle cx="7.5" cy="1.5" r="1.0" fill="#6B4D10"/>
        <circle cx="5" cy="6.5" r="1.6" fill="#6B4D10"/>
        <circle cx="9" cy="5" r="1.1" fill="#6B4D10"/>
        <circle cx="1" cy="7" r="0.7" fill="#5A3D08"/>
      </pattern>
      <pattern id="mod-trazas" width="14" height="10" patternUnits="userSpaceOnUse">
        <path d="M0,5 Q2,2 4,5 Q6,8 8,5 Q10,2 12,5" stroke="#5C3D1A" strokeWidth="1.4" fill="none"/>
        <circle cx="0" cy="5" r="1" fill="#5C3D1A"/>
        <circle cx="12" cy="5" r="1" fill="#5C3D1A"/>
      </pattern>
      <pattern id="mod-bioturbacion" width="12" height="12" patternUnits="userSpaceOnUse">
        <ellipse cx="6" cy="4" rx="4" ry="2" stroke="#7A5830" strokeWidth="1" fill="none"/>
        <line x1="6" y1="2" x2="6" y2="10" stroke="#7A5830" strokeWidth="0.8"/>
        <ellipse cx="3" cy="9" rx="2.5" ry="1.5" stroke="#7A5830" strokeWidth="0.8" fill="none"/>
      </pattern>
      <pattern id="mod-oxidacion" width="8" height="8" patternUnits="userSpaceOnUse">
        <line x1="0" y1="4" x2="8" y2="4" stroke="#C0391B" strokeWidth="0.8"/>
        <line x1="4" y1="0" x2="4" y2="8" stroke="#C0391B" strokeWidth="0.5"/>
        <circle cx="4" cy="4" r="1" fill="#C0391B"/>
      </pattern>
      <pattern id="mod-cemento-calc" width="10" height="8" patternUnits="userSpaceOnUse">
        <line x1="0" y1="0" x2="10" y2="0" stroke="#7AAEDB" strokeWidth="0.8"/>
        <line x1="0" y1="4" x2="10" y2="4" stroke="#7AAEDB" strokeWidth="0.5"/>
        <circle cx="5" cy="2" r="0.9" fill="#2D7BA8"/>
      </pattern>
      <pattern id="mod-silice" width="10" height="10" patternUnits="userSpaceOnUse">
        <polygon points="5,1 9,8 1,8" stroke="#9A8E78" strokeWidth="0.9" fill="none"/>
        <polygon points="5,3 7.5,7.5 2.5,7.5" fill="#9A8E78" fillOpacity="0.3"/>
      </pattern>
      <pattern id="mod-nodulos" width="14" height="10" patternUnits="userSpaceOnUse">
        <ellipse cx="4" cy="5" rx="3" ry="2.5" stroke="#6B5C3E" strokeWidth="1" fill="#B8A882" fillOpacity="0.45"/>
        <ellipse cx="11" cy="5" rx="2.5" ry="2" stroke="#6B5C3E" strokeWidth="0.8" fill="#B8A882" fillOpacity="0.35"/>
      </pattern>
      <pattern id="mod-carbon" width="8" height="6" patternUnits="userSpaceOnUse">
        <rect width="8" height="6" fill="#2C2C2A"/>
        <line x1="0" y1="2" x2="8" y2="2" stroke="#5F5E5A" strokeWidth="0.5"/>
        <line x1="0" y1="4.5" x2="8" y2="4.5" stroke="#B8A882" strokeWidth="0.3"/>
      </pattern>
    </defs>
  );
}
...
// ═══════════════════════════════════════════════════════════════
// LITHOLOGY PICKER — dropdown con grupos colapsables
// ═══════════════════════════════════════════════════════════════
function LithoPicker({ value, onChange, exclude }) {
  const [open, setOpen] = useState(false);
  // El onClick del trigger re-siembra `expanded` en cada apertura; basta un Set vacío inicial
  ...
  const li = findL(value);
  const toggle = g => setExpanded(p => {
    const n = new Set(p); if (n.has(g)) n.delete(g); else n.add(g); return n;
  });
  return (
    <div ref={ref} style={{ position: "relative" }}>
      <button type="button" onClick={() => { if (!open) { setExpanded(new Set(li ? [li[4]] : [])); } setOpen(!open); }}
        style={{
          width: "100%", textAlign: "left", padding: "6px 8px",
          fontFamily: MONO, fontSize: 11.5,
          border: `1px solid ${T.inputBd}`, background: T.inputBg,
          color: T.text, borderRadius: 3, cursor: "pointer",
          display: "flex", justifyContent: "space-between", alignItems: "center",
        }}>
        <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{li[1]}</span>
        <span style={{ fontSize: 9, color: T.text3, flexShrink: 0, marginLeft: 4 }}>{open ? "▲" : "▼"}</span>
      </button>
      {open && (
        <div style={{
          position: "absolute", top: "100%", left: 0, right: 0, zIndex: 100,
          maxHeight: 340, overflowY: "auto",
          background: T.card, border: `1px solid ${T.borderDk}`,
          borderRadius: "0 0 4px 4px", boxShadow: "0 6px 16px rgba(42,31,18,.18)",
        }}>
          {LITH_GROUPS.map(g => {
            const items = exclude
              ? LITH_BY_GROUP.get(g).filter(l => l[0] !== exclude)
              : LITH_BY_GROUP.get(g);
            if (items.length === 0) return null;
            const isExp = expanded.has(g);
            return (
              <div key={g}>
                <div onClick={() => toggle(g)} style={{
                  padding: "9px 10px", cursor: "pointer",
                  background: T.panelAlt, fontFamily: SANS, fontSize: 10,
                  fontWeight: 700, color: T.borderDk, borderBottom: `1px solid ${T.border}`,
                  display: "flex", justifyContent: "space-between", alignItems: "center",
                  userSelect: "none", position: "sticky", top: 0, zIndex: 1,
                  letterSpacing: ".02em",
                }}>
                  <span>{g}</span>
                  <span style={{ fontSize: 8, color: T.text3 }}>{isExp ? "▼" : "▶"} {items.length}</span>
                </div>
                {isExp && items.map(l => (
                  <div key={l[0]}
                    onClick={() => { onChange(l[0]); setOpen(false); }}
                    style={{
                      padding: "9px 14px", cursor: "pointer", fontSize: 12,
                      fontFamily: MONO, color: l[0] === value ? T.green : T.text,
                      fontWeight: l[0] === value ? 700 : 400,
                      background: l[0] === value ? "#E3EEDD" : "transparent",
                      borderBottom: `1px solid ${T.border}`,
                    }}
                    onMouseEnter={e => { if (l[0] !== value) e.currentTarget.style.background = T.panelAlt; }}
                    onMouseLeave={e => { if (l[0] !== value) e.currentTarget.style.background = "transparent"; }}
                  >{l[1]}</div>
                ))}
              </div>
            );
...
const DF = {
  ...
  age: "", formation: "",
  // Modo de cálculo del espesor: "badgley" o "manual"
  thicknessMode: "badgley",
  ...
  interpLi2MinCm: "", interpLi2MaxCm: "",  // rango de espesor de la litología secundaria
  beddingTrend: "",  // "" = uniforme, "thinning" = estrato decreciente (thins upward)
  photo: null, photoCaption: "",
  modTecho: null, modMuro: null,
  bioturbIndex: null, traceIchnos: [],
};
const DEFAULT_META = {
  project: "", locality: "", author: "", date: "",
  datum: "WGS84", utmZone: "19", utmHemi: "S",
  utmE: "", utmN: "",            // base / inicio de la sección
  utmE2: "", utmN2: "",          // techo / fin de la sección
  elevation: "", azimuth: "", description: "",
};
let nextId = 1;
...
function ColumnaEstratigrafica() {
  ...
  const [openIchnoGroups, setOpenIchnoGroups] = useState(() => new Set(["Skolithos"]));
  const toggleIchnoGroup = g => setOpenIchnoGroups(prev => {
    const next = new Set(prev);
    next.has(g) ? next.delete(g) : next.add(g);
    return next;
  });
  const toggleIchno = id => setF(p => {
    const cur = p.traceIchnos || [];
    return { ...p, traceIchnos: cur.includes(id) ? cur.filter(x => x !== id) : [...cur, id] };
  });
  useEffect(() => {
  ...
  const usedMods = MOD_TYPES.filter(m => units.some(u => u.modTecho?.type === m[0] || u.modMuro?.type === m[0]));
  const modRows = Math.ceil(usedMods.length / 2);
  const usedIchno = ICHNO_TYPES.filter(t => units.some(u => (u.traceIchnos || []).includes(t[0])));
  const ichnoRows = Math.ceil(usedIchno.length / 2);
  const hasBI = units.some(u => u.bioturbIndex != null);
  ...
  const modsTopY = legCur + 14;
  if (usedMods.length) legCur = modsTopY + 18 + modRows * 17;
  const ichnoTopY = legCur + 14;
  if (usedIchno.length) legCur = ichnoTopY + 18 + ichnoRows * 17;
  const biTopY = legCur + 14;
  if (hasBI) legCur = biTopY + 34;
  const legH = 24 + legCur + 16;
  const svgH     = MT + totalH * SPX + legH;
  const svgW     = SW;
  const _e1 = parseFloat(meta.utmE), _n1 = parseFloat(meta.utmN);
  const _e2 = parseFloat(meta.utmE2), _n2 = parseFloat(meta.utmN2);
  const sectionAz = [_e1, _n1, _e2, _n2].every(Number.isFinite) ? utmBearing(_e1, _n1, _e2, _n2) : null;
  const metaUtm  = (meta.utmE && meta.utmN)
    ? `UTM ${meta.utmZone || ""}${meta.utmHemi || ""} ${meta.utmE}E / ${meta.utmN}N`
      + (sectionAz != null ? ` · Az ${Math.round(sectionAz)}°` : "") : "";
  ...
  // ── Handlers ─────────────────────────────────────────────────
  // El espesor que dibuja la columna depende del modo:
  //  - "manual"  → toma directamente u.thickness (m)
  //  - "badgley" → calcula con Badgley/Ragan; si faltan datos, cae a u.thickness
  const realFor = u => {
    if (u.thicknessMode === "manual") {
  ...
  const doEdit   = u => {
    ...
    const ichnos = (u.traceIchnos || []);
    const activeIchnoGroups = new Set(ichnos.map(id => {
      const t = ICHNO_TYPES.find(t => t[0] === id);
      return t ? t[2] : null;
    }).filter(Boolean));
    setOpenIchnoGroups(activeIchnoGroups.size > 0 ? activeIchnoGroups : new Set(["Skolithos"]));
  };
  ...
  // Duplica una unidad insertando la copia justo encima (útil en ritmitas)
  const doDup = id => setUnits(p => {
    const i = p.findIndex(u => u.id === id);
    if (i < 0) return p;
    const a = [...p];
    a.splice(i + 1, 0, { ...p[i], id: `u${nextId++}` });
    return a;
  });
  const doClear = () => {
  ...
  const onCsvChosen = async e => {
    ...
    const parsed = rows.map(r => {
      ...
      const lid = normalizeLithoId(r.lithology);
      const lithoId = validLitho.has(lid) ? lid : "sandstone";
      const lid2 = normalizeLithoId(r.lithology2);
      ...
      return {
        ...
        thicknessMode: r.thicknessMode === "manual" ? "manual" : "badgley",
        interpLi1MinCm: r.interpLi1MinCm || "", interpLi1MaxCm: r.interpLi1MaxCm || "",
        interpLi2MinCm: r.interpLi2MinCm || "", interpLi2MaxCm: r.interpLi2MaxCm || "",
        beddingTrend: r.beddingTrend === "thinning" ? "thinning" : "",
        photo: null, photoCaption: "",
        modTecho: parseMod(r.modTechoType, r.modTechoCm, r.modTechoNotes, t),
        modMuro:  parseMod(r.modMuroType,  r.modMuroCm,  r.modMuroNotes,  t),
        bioturbIndex: (r.bioturbIndex !== "" && r.bioturbIndex != null)
          ? Math.min(6, Math.max(0, parseInt(r.bioturbIndex, 10) || 0)) : null,
        traceIchnos: r.traceIchnos
          ? r.traceIchnos.split(";").map(s => s.trim()).filter(s => ICHNO_TYPES.some(x => x[0] === s))
          : [],
      };
    }).filter(Boolean);
  ...
  const exportKmz = async () => {
    ...
    const start = utmToLatLon(E, N, zone, isSouth);
    const E2 = parseFloat(meta.utmE2), N2 = parseFloat(meta.utmN2);
    const hasEnd = isFinite(E2) && isFinite(N2);
    const end = hasEnd ? utmToLatLon(E2, N2, zone, isSouth) : null;
    const az = hasEnd ? utmBearing(E, N, E2, N2)
      : (isFinite(parseFloat(meta.azimuth)) ? parseFloat(meta.azimuth) : null);
    let pngBytes = null;
    try {
      const svgNode = document.getElementById("colsvg");
      if (svgNode) pngBytes = dataUrlToBytes(await svgToPngDataUrl(svgNode, 2));
    } catch { /* la imagen es opcional */ }
    const kml = buildKml(meta, units, start, end, az, !!pngBytes);
    ...
    showToast(`✓ KMZ exportado${az != null ? ` · azimut ${Math.round(az)}°` : ""}`);
  };
  ...
  const onJsonChosen = async e => {
    ...
    try {
      ...
      const parsed = arr.map(u => ({
        ...DF, ...u, id: `u${nextId++}`,
        lithoId: normalizeLithoId(u.lithoId) || DF.lithoId,
        ...
        traceIchnos:   Array.isArray(u.traceIchnos) ? u.traceIchnos.filter(s => ICHNO_TYPES.some(t => t[0] === s)) : [],
        bioturbIndex:  u.bioturbIndex != null ? Math.min(6, Math.max(0, parseInt(u.bioturbIndex, 10) || 0)) : null,
      ...
      setSamples(Array.isArray(data.samples)
        ? data.samples.map(s => {
            const o = { id: `s${nextId++}`, height: parseFloat(s.height) || 0,
              code: String(s.code || ""), type: validST.has(s.type) ? s.type : "otra" };
            if (s.utmE != null && s.utmN != null && String(s.utmE).trim() && String(s.utmN).trim()) {
              o.utmE = String(s.utmE); o.utmN = String(s.utmN);
            }
            return o;
          }).filter(s => s.code)
        : []);
  ...
  // Núcleo GPS reutilizable: pide posición y entrega UTM ya convertido a onOk(u, coords).
  const requestGpsUtm = onOk => {
  ...
  const captureGps = which => requestGpsUtm((u, coords) => {
    ...
    setMeta(p => ({
      ...
      ...(which === "base" && coords.altitude != null && !p.elevation
        ? { elevation: coords.altitude.toFixed(0) } : {}),
    }));
  ...
  // En móvil agrandamos el target táctil (mín. ~34 px de alto) sin tocar el desktop
  const btnSmall = { ...btnBase, fontSize: isNarrow ? 12 : 10,
    padding: isNarrow ? "7px 12px" : "2px 7px",
    minHeight: isNarrow ? 34 : undefined };
  // Botón de grupo toggle (selección excluyente): verde si activo
  const tgStyle = active => ({
    flex: 1, cursor: "pointer", borderRadius: 3,
    background: active ? "#E3EEDD" : T.inputBg,
    border: `1px solid ${active ? T.green : T.inputBd}`,
    ...
    <div style={{
      ...
      <div style={{ display: "flex", flex: 1, minHeight: 0,
        ...
        <div style={{
          ...
          paddingBottom: isNarrow ? "env(safe-area-inset-bottom)" : undefined,
        }}>
          {/* TAB BAR */}
          <div style={{ display: "flex", borderBottom: `1px solid ${T.border}`,
                        background: T.panel, flexShrink: 0 }}>
            {PANEL_TABS.map(([id, lbl]) => (
          ...
          {panelTab === "samples" && (
            <SamplesForm samples={samples} addSample={addSample} delSample={delSample}
              ...
              secStyle={secStyle} btnPrimary={btnPrimary} btnSmall={btnSmall} btnBase={btnBase}
              requestGpsUtm={requestGpsUtm} meta={meta} />
          )}
          ...
          <div style={{
            ...
            <span style={{ color: editId ? T.rust : T.indigo, fontSize: 16 }}>📐</span>
            {editId ? "Espesor — editando unidad" : "Espesor — nueva unidad"}
          </div>
          <div style={{ padding: "12px 16px", display: "flex",
                        flexDirection: "column", gap: 9, flex: 1 }}>

            <div style={{ ...secStyle, marginTop: 0, paddingTop: 0,
                          borderTop: "none" }}>· Modo de cálculo ·</div>

            <div style={lblStyle}>
              <div style={{ display: "flex", gap: 6 }}>
                {[["badgley", "🧮 Calcular (Badgley)", "rumbo, manteo, dh y Δh"],
                  ["manual", "✍️ Ingresar manual", "medición directa (ej. ritmitas)"]].map(([v, l, d]) => (
                  <button key={v} type="button" onClick={() => ff("thicknessMode", v)}
                    style={{ ...tgStyle(f.thicknessMode === v), padding: "7px 8px",
                      fontSize: 10.5, textAlign: "left", lineHeight: 1.35 }}>
                    <div>{l}</div>
                    <div style={{ fontSize: 9, color: T.text3, fontFamily: SERIF,
                      fontStyle: "italic", marginTop: 1 }}>{d}</div>
                  </button>
                ))}
              </div>
            </div>

            {f.thicknessMode === "manual" ? (<>
              <label style={lblStyle}>Espesor real (m)
                <input type="number" min="0.001" step="0.01" placeholder="ej: 0.50  (50 cm)"
                ...
                <span style={{ fontSize: 9.5, color: T.text3, fontStyle: "italic", fontFamily: SERIF }}>
                  Ideal para ritmitas finas o cuando la columna está medida con flexómetro
                  contra el afloramiento.
                </span>
              </label>
            </>) : (<>
                  ...
                  <div style={{ display: "flex", gap: 4, marginTop: 3 }}>
                    {[["derecha", "Der. (+90°)"], ["izquierda", "Izq. (−90°)"]].map(([v, l]) => (
                      <button key={v} type="button" onClick={() => ff("dirBuz", v)}
                        style={{ ...tgStyle(f.dirBuz === v), padding: "6px 4px",
            ...
            <div style={secStyle}>· Resultado ·</div>
            {(() => {
              const t = realFor(f);
              if (t != null && t > 0) {
                const g = f.thicknessMode !== "manual" ? unitGeometry(f) : null;
                return (
                  <div style={{ padding: "10px 12px",
                    background: (g && g.espesor < 0) ? "#F8D8CC" : "#E3EEDD",
                    border: `1px solid ${(g && g.espesor < 0) ? T.danger : T.green}`,
                    borderRadius: 4, fontSize: 12, fontFamily: MONO, color: T.text }}>
                    <div>Espesor real ≈ <b style={{ fontSize: 16 }}>{t.toFixed(2)} m</b>
                      <span style={{ fontSize: 10, color: T.text3, marginLeft: 6 }}>
                        ({f.thicknessMode === "manual" ? "manual" : "Badgley"})
                      </span>
                    </div>
                    {g && g.espesor < 0 && (
              ...
              return (
                <div style={{ padding: "8px 10px", background: T.panelAlt,
                  border: `1px dashed ${T.border}`, borderRadius: 3,
                  fontSize: 10.5, color: T.text3, fontFamily: SERIF, fontStyle: "italic" }}>
                  {f.thicknessMode === "manual"
                    ? "Ingresá el espesor en metros arriba."
                    : "Cargá rumbo, manteo, dh y Δh para calcular el espesor real."}
                </div>
              );
            })()}
          </div>
          ...
          <div style={{ padding: "12px 16px", display: "flex",
            ...
            {/* Chip de espesor — conectado al tab Espesor */}
            {(() => {
              const t = realFor(f);
              const hasVal = t != null && t > 0;
              return (
                <button type="button" onClick={() => setPanelTab("thickness")}
                  style={{
                    width: "100%", padding: "9px 12px", cursor: "pointer",
                    background: hasVal ? "#E3EEDD" : "#FFF3E0",
                    border: `1px solid ${hasVal ? T.green : T.gold}`,
                    borderRadius: 4, display: "flex", alignItems: "center",
                    justifyContent: "space-between", gap: 8,
                    transition: "all .12s",
                  }}>
                  <span style={{ fontFamily: MONO, fontSize: 12, color: T.text, fontWeight: 600 }}>
                    {hasVal
                      ? <>📐 Espesor: {t.toFixed(2)} m <span style={{ fontWeight: 400, fontSize: 10, color: T.text3 }}>({f.thicknessMode === "manual" ? "manual" : "Badgley"})</span></>
                      : <>⚠ Sin espesor — configurar</>}
                  </span>
                  <span style={{ fontSize: 11, color: hasVal ? T.green : T.gold, fontWeight: 600 }}>→</span>
                </button>
              );
            })()}

            <div style={secStyle}>· Litología ·</div>

            <div style={lblStyle}>
              <span>Litología</span>
              <LithoPicker value={f.lithoId} onChange={id => {
                const li = findL(id);
                ff("lithoId", id); ff("grainSize", li[3]);
                if (id === f.lithoId2) ff("lithoId2", "");
              }} />
            </div>
            ...
            <div style={{
              ...
              {f.lithoId2 && (
                <div style={{ marginTop: 8, display: "flex",
                  ...
                  <div style={lblStyle}>
                    <span>Litología secundaria</span>
                    <LithoPicker value={f.lithoId2}
                      onChange={id => ff("lithoId2", id)}
                      exclude={f.lithoId} />
                  </div>
                  ...
                  <label style={{ ...lblStyle, flexDirection: "row",
                    alignItems: "center", gap: 7, cursor: "pointer", marginTop: 4 }}>
                    <input type="checkbox" checked={f.beddingTrend === "thinning"}
                      onChange={e => ff("beddingTrend", e.target.checked ? "thinning" : "")}
                      style={{ cursor: "pointer" }} />
                    <span style={{ color: f.beddingTrend === "thinning" ? T.rust : T.text2,
                      fontStyle: "italic", fontSize: 10.5 }}>
                      Estrato decreciente hacia arriba
                    </span>
                  </label>
                  <div style={{ marginTop: 6, padding: "7px 9px", background: T.card,
                    border: `1px solid ${T.cardEdge}`, borderRadius: 3 }}>
                    <div style={{ fontSize: 9.5, color: T.text2, fontFamily: SANS,
                      fontWeight: 600, marginBottom: 5 }}>Espesores individuales (cm)</div>
                    <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                      <span style={{ fontSize: 9, color: T.text3, whiteSpace: "nowrap", minWidth: 22 }}>Li₁</span>
                      <input type="number" min="0" step="0.1" placeholder="mín"
                        value={f.interpLi1MinCm} onChange={e => ff("interpLi1MinCm", e.target.value)}
                        style={{ ...inputStyle, flex: 1, padding: "4px 6px", fontSize: 10.5 }} />
                      <span style={{ fontSize: 9, color: T.text3 }}>–</span>
                      <input type="number" min="0" step="0.1" placeholder="máx"
                        value={f.interpLi1MaxCm} onChange={e => ff("interpLi1MaxCm", e.target.value)}
                        style={{ ...inputStyle, flex: 1, padding: "4px 6px", fontSize: 10.5 }} />
                    </div>
                    <div style={{ display: "flex", gap: 6, alignItems: "center", marginTop: 4 }}>
                      <span style={{ fontSize: 9, color: T.text3, whiteSpace: "nowrap", minWidth: 22 }}>Li₂</span>
                      <input type="number" min="0" step="0.1" placeholder="mín"
                        value={f.interpLi2MinCm} onChange={e => ff("interpLi2MinCm", e.target.value)}
                        style={{ ...inputStyle, flex: 1, padding: "4px 6px", fontSize: 10.5 }} />
                      <span style={{ fontSize: 9, color: T.text3 }}>–</span>
                      <input type="number" min="0" step="0.1" placeholder="máx"
                        value={f.interpLi2MaxCm} onChange={e => ff("interpLi2MaxCm", e.target.value)}
                        style={{ ...inputStyle, flex: 1, padding: "4px 6px", fontSize: 10.5 }} />
                    </div>
                  </div>
                </div>
            ...
            <div style={lblStyle}>
              <span>Estructuras — símbolos
                {(f.structSymbols || []).length > 0 && (
                  <span style={{ marginLeft: 6, color: T.green, fontSize: 10, fontWeight: 700 }}>
                    {(f.structSymbols || []).length} sel.
                  </span>
                )}
              </span>
            ...
            <div style={lblStyle}>
              <span>Índice de bioturbación (BI)
                {f.bioturbIndex != null && (
                  <span style={{ marginLeft: 6, color: T.indigo, fontSize: 10, fontWeight: 700 }}>
                    BI = {f.bioturbIndex}
                  </span>
                )}
              </span>
              <div style={{ display: "flex", gap: 3, marginTop: 3 }}>
                {[null,0,1,2,3,4,5,6].map(v => {
                  const lbl = v == null ? "–" : String(v);
                  const on  = f.bioturbIndex === v;
                  return (
                    <button key={lbl} type="button"
                      onPointerDown={e => { e.preventDefault(); ff("bioturbIndex", v); }}
                      style={{ flex: 1, padding: "5px 2px", fontSize: 11, fontFamily: MONO,
                        border: `1px solid ${on ? T.indigo : T.border}`,
                        borderRadius: 3, cursor: "pointer", minHeight: 30,
                        background: on ? "#DDEAF5" : T.inputBg,
                        color: on ? T.indigo : T.text2, fontWeight: on ? 700 : 400 }}>
                      {lbl}
                    </button>
                  );
                })}
              </div>
              <span style={{ fontSize: 9, color: T.text3, marginTop: 2 }}>
                0 sin bioturb. · 3 ~50% · 6 completa (Droser &amp; Bottjer 1986)
              </span>
            </div>

            <div style={lblStyle}>
              <span>Trazas fósiles / Icnofácies
                {(f.traceIchnos || []).length > 0 && (
                  <span style={{ marginLeft: 6, color: T.indigo, fontSize: 10, fontWeight: 700 }}>
                    {(f.traceIchnos || []).length} sel.
                  </span>
                )}
              </span>
              <div style={{ marginTop: 3, border: `1px solid ${T.border}`, borderRadius: 3,
                background: T.inputBg }}>
                {ICHNO_GROUPS.map(g => {
                  const groupItems = ICHNO_TYPES.filter(t => t[2] === g);
                  const activeInGroup = groupItems.filter(t => (f.traceIchnos || []).includes(t[0])).length;
                  const isOpen = openIchnoGroups.has(g);
                  return (
                    <div key={g}>
                      <button type="button"
                        onPointerDown={e => { e.preventDefault(); toggleIchnoGroup(g); }}
                        style={{ display: "flex", alignItems: "center", justifyContent: "space-between",
                          width: "100%", padding: "5px 8px", background: T.panelAlt,
                          border: "none", borderBottom: `1px solid ${T.border}`,
                          cursor: "pointer", textAlign: "left", minHeight: 34 }}>
                        <span style={{ fontSize: 9, fontWeight: 700, color: T.indigo,
                        ...
                        <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
                          {activeInGroup > 0 && (
                            <span style={{ fontSize: 9, fontWeight: 700, color: T.indigo,
                              background: "#DDEAF5", borderRadius: 8, padding: "1px 5px" }}>
                              {activeInGroup}
                            </span>
                          )}
                          <span style={{ fontSize: 10, color: T.text3 }}>{isOpen ? "▲" : "▼"}</span>
                      ...
                      {isOpen && groupItems.map(t => {
                        const on = (f.traceIchnos || []).includes(t[0]);
                        return (
                          <label key={t[0]} style={{ display: "flex", alignItems: "center",
                            gap: 7, padding: "5px 8px", cursor: "pointer",
                            background: on ? "#DDEAF5" : "transparent",
                            borderBottom: `1px solid ${T.cardEdge}`,
                            minHeight: 34 }}>
                            <input type="checkbox" checked={on}
                              onChange={() => toggleIchno(t[0])}
                              style={{ cursor: "pointer", accentColor: T.indigo, width: 16, height: 16 }} />
                            <IchnoIcon id={t[0]} size={18} />
                            <span style={{ fontSize: 11, color: on ? T.indigo : T.text,
                              fontWeight: on ? 600 : 400, flex: 1 }}>{t[1]}</span>
                          </label>
                        );
                      })}
                    </div>
            ...
            {(f => {
              const hasMod = f.modTecho?.type || f.modMuro?.type;
              const maxCm = Math.floor((parseFloat(f.thickness) || 0) * 100);
              const modRow = (pos, label) => {
                const key = pos === "techo" ? "modTecho" : "modMuro";
                const val = f[key] || {};
                const setField = (field, v) => ff(key, { ...val, [field]: v });
                const setType  = t => ff(key, t ? { type: t, cm: val.cm || Math.max(1, Math.round(maxCm * 0.15)), notes: val.notes || "" } : null);
                return (
                  <div key={pos} style={{ display: "flex", gap: 5, alignItems: "center", marginBottom: 5, flexWrap: "wrap" }}>
                    <span style={{ fontSize: 10, color: T.text3, minWidth: 36 }}>{label}</span>
                    <select value={val.type || ""} onChange={e => setType(e.target.value || null)}
                      style={{ ...inputStyle, flex: 1, minWidth: 120, padding: "3px 5px" }}>
                      <option value="">— sin modificador —</option>
                      {MOD_TYPES.map(m => <option key={m[0]} value={m[0]}>{m[1]}</option>)}
                    </select>
                    {val.type && <>
                      <input type="number" min="1" max={maxCm || 1} value={val.cm || ""}
                        onChange={e => setField("cm", Math.min(Math.max(1, parseInt(e.target.value) || 1), maxCm || 1))}
                        style={{ ...inputStyle, width: 56 }} placeholder="cm" title={`máx ${maxCm} cm`} />
                      <span style={{ fontSize: 9, color: T.text3 }}>cm</span>
                      <input type="text" value={val.notes || ""} maxLength={120}
                        onChange={e => setField("notes", e.target.value)}
                        style={{ ...inputStyle, flex: 2, minWidth: 80 }} placeholder="notas…" />
                    </>}
                  </div>
                );
              };
              return (
                <div style={{ marginTop: 8, border: `1px solid ${T.border}`, borderRadius: 4 }}>
                  <button type="button" onClick={() => setModOpen(o => !o)} style={{
                    width: "100%", background: T.panel, border: "none", textAlign: "left",
                    padding: "5px 10px", cursor: "pointer", fontSize: 10.5,
                    color: T.text2, fontFamily: SANS, display: "flex", justifyContent: "space-between" }}>
                    <span>⚗ Modificadores de zona{hasMod ? " ●" : ""}</span>
                    <span>{modOpen ? "▲" : "▼"}</span>
                  </button>
                  {modOpen && (
                    <div style={{ padding: "8px 10px" }}>
                      {maxCm < 1 && <div style={{ fontSize: 9.5, color: T.text3, marginBottom: 6 }}>
                        Ingresá el espesor de la unidad primero.</div>}
                      {modRow("techo", "Techo")}
                      {modRow("muro",  "Base")}
                    </div>
                  )}
                  ...
                  <div key={u.id} style={{
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
                        ...
                        <button onClick={() => doDup(u.id)} style={btnSmall}
                          title="Duplicar (copia encima)">⧉</button>
        ...
        <div style={{ flex: 1, overflow: "auto",
                      ...
                      paddingBottom: isNarrow ? "calc(12px + env(safe-area-inset-bottom))" : undefined,
          ...
          {units.length === 0 ? (
            <div style={{
              ...
              <div style={{ fontSize: 12, lineHeight: 1.7 }}>
                {isNarrow
                  ? <>Ingrese unidades en el panel de la izquierda<br/>de <i>base → techo</i> para construir la columna.</>
            ...
            <div id="printarea" style={{
              ...
              <svg id="colsvg" width={svgW} height={svgH} xmlns="http://www.w3.org/2000/svg"
                ...
                {/* Pista de índice de bioturbación */}
                <text x={BTX + BTW / 2} y={MT - 16} textAnchor="middle" style={{
                  fontSize: 8, fontFamily: MONO, fill: T.indigo, fontStyle: "italic" }}>BI</text>
                <rect x={BTX} y={MT} width={BTW} height={totalH * SPX}
                  fill="#F4F8FC" stroke={T.border} strokeWidth="0.8" />

                {/* Pista de icnofósiles */}
                <text x={ITX + ITW / 2} y={MT - 16} textAnchor="middle" style={{
                  fontSize: 8, fontFamily: MONO, fill: T.indigo, fontStyle: "italic" }}>icno</text>
                <rect x={ITX} y={MT} width={ITW} height={totalH * SPX}
                  fill="#F4F8FC" stroke={T.border} strokeWidth="0.8" />
                ...
                <g clipPath="url(#colclip)">
                  {units.map((u, i) => {
                    ...
                    const isCov = u.lithoId === "covered";
                    const uw   = isCov ? CW * GRAIN[0][2] : CW * gi[2];
                    const hasInter = u.lithoId2 && u.lithoId2 !== u.lithoId;
                    const mw = hasInter ? Math.max(uw, CW * findG(findL(u.lithoId2)[3])[2]) : uw;
                    const modOverlays = (() => {
                      const els = [];
                      const maxCm = u.thickness * 100;
                      if (u.modTecho?.type) {
                        const mT = MOD_TYPES.find(m => m[0] === u.modTecho.type);
                        if (mT) {
                          const hm = Math.min(u.modTecho.cm, maxCm) / 100 * SPX;
                          els.push(
                            <rect key="mt" x={CX} y={topY} width={mw} height={hm}
                              fill={`url(#${mT[2]})`} fillOpacity="0.72"/>,
                            <line key="mtl" x1={CX} y1={topY + hm} x2={CX + mw} y2={topY + hm}
                              stroke="#5C3D1A" strokeWidth="0.8" strokeDasharray="3,2"/>
                          );
                        }
                      }
                      if (u.modMuro?.type) {
                        const mM = MOD_TYPES.find(m => m[0] === u.modMuro.type);
                        if (mM) {
                          const hm = Math.min(u.modMuro.cm, maxCm) / 100 * SPX;
                          els.push(
                            <rect key="mm" x={CX} y={topY + hpx - hm} width={mw} height={hm}
                              fill={`url(#${mM[2]})`} fillOpacity="0.72"/>,
                            <line key="mml" x1={CX} y1={topY + hpx - hm} x2={CX + mw} y2={topY + hpx - hm}
                              stroke="#5C3D1A" strokeWidth="0.8" strokeDasharray="3,2"/>
                          );
                        }
                      }
                    ...
                    if (!hasInter) {
                      // Cubierto / sin afloramiento → franja angosta, fondo blanco, cruz
                      if (isCov) {
                        return (
                          <g key={u.id}>
                            <rect x={CX} y={topY} width={CW} height={hpx}
                              fill={T.card} opacity="0.94" />
                            <rect x={CX} y={topY} width={uw} height={hpx}
                              fill="white" stroke={T.border} strokeWidth="0.6" />
                            {hpx > 10 && <>
                              <line x1={CX + 2} y1={topY + 2} x2={CX + uw - 2} y2={topY + hpx - 2}
                                stroke={T.text3} strokeWidth="0.9" />
                              <line x1={CX + uw - 2} y1={topY + 2} x2={CX + 2} y2={topY + hpx - 2}
                                stroke={T.text3} strokeWidth="0.9" />
                            </>}
                          </g>
                        );
                      }
                      if (edgeStyle === "freehand") {
                        const xr = Math.min(CX + uw, CX + CW - EDGE_MARGIN);
                        return (
                          <g key={u.id}>
                            <rect x={CX} y={topY} width={CW} height={hpx}
                              fill={T.card} opacity="0.94" />
                            <path d={freehandUnitPath(CX, xr, topY, hpx, i + 1)}
                              fill={`url(#${li[2]})`} />
                            {modOverlays}
                          </g>
                        );
                    ...
                    // Cada litología conserva el ancho de SU granulometría → perfil dentado.
                    const li2    = findL(u.lithoId2);
                    const ratio  = u.lithoRatio ?? 0.5;
                    const uwA    = uw;
                    const uwB    = CW * findG(li2[3])[2];
                    const nCyc   = Math.max(2, Math.min(28, Math.floor(hpx / 30)));
                    const isThin = u.beddingTrend === "thinning";
                    // Estrato decreciente: k=0 (techo, delgado) → k=nCyc-1 (base, grueso)
                    // Peso de banda k: (k+1) — crece hacia la base.
                    const totalW = isThin ? nCyc * (nCyc + 1) / 2 : 0;
                    const bands  = [];
                    const fh = edgeStyle === "freehand";
                    let yPos = topY;
                    for (let k = 0; k < nCyc; k++) {
                      const cycH_k = isThin ? hpx * (k + 1) / totalW : hpx / nCyc;
                      const yStart = yPos;
                      yPos += cycH_k;
                      const aH     = cycH_k * ratio;
                      const bhA = aH, bhB = cycH_k - aH;
                      const fsA = Math.max(1, Math.min(2.8, 26 / Math.max(2, bhA)));
                      const fsB = Math.max(1, Math.min(2.8, 26 / Math.max(2, bhB)));
                      const asA = Math.max(0.7, Math.min(1, bhA / 22));
                      const asB = Math.max(0.7, Math.min(1, bhB / 22));
                      bands.push(
                        fh
                          ? <path key={`${u.id}-a${k}`} fill={`url(#${li[2]})`}
                              d={freehandUnitPath(CX, Math.min(CX + uwA, CX + CW - EDGE_MARGIN), yStart, aH, i + 1, fsA, asA)} />
                          : <rect key={`${u.id}-a${k}`} x={CX} y={yStart}
                              width={uwA} height={aH} fill={`url(#${li[2]})`} />,
                        fh
                          ? <path key={`${u.id}-b${k}`} fill={`url(#${li2[2]})`}
                              d={freehandUnitPath(CX, Math.min(CX + uwB, CX + CW - EDGE_MARGIN), yStart + aH, cycH_k - aH, i + 101, fsB, asB)} />
                          : <rect key={`${u.id}-b${k}`} x={CX} y={yStart + aH}
                              width={uwB} height={cycH_k - aH} fill={`url(#${li2[2]})`} />,
                ...
                {units.map((u, i) => {
                  if (i === 0) return null;
                  const y = yOf(cumH[i]);
                  // Ancho del contacto = max entre la unidad inferior (i) y la superior (i-1)
                  const uwU = u.lithoId === "covered" ? CW * GRAIN[0][2] : CW * findG(u.grainSize)[2];
                  const uwP = units[i-1].lithoId === "covered" ? CW * GRAIN[0][2] : CW * findG(units[i-1].grainSize)[2];
                  const cx2 = CX + Math.max(uwU, uwP);
                  return (
                    <path key={`c${u.id}`} d={cPath(u.contactBottom, CX, cx2, y)}
                      fill="none" stroke={cClr(u.contactBottom)}
                      strokeWidth={cWid(u.contactBottom)} strokeLinecap="round"
                      strokeDasharray={cDash(u.contactBottom)} />
                  );
                })}
                ...
                <g clipPath="url(#colclip2)">
                  {units.map((u, i) => {
                    ...
                    const gs   = 20;
                    const cx   = STX + STW / 2;
                    // Gradación normal/inversa → triángulo de altura completa del estrato
                    const hasGN = syms.includes("graded_normal");
                    const hasGR = syms.includes("graded_reverse");
                    const others = syms.filter(s => s !== "graded_normal" && s !== "graded_reverse");
                    const maxN = Math.max(1, Math.floor(hpx / gs));
                    const shown = others.slice(0, maxN);
                    const y0 = midY - (shown.length - 1) * gs / 2;
                    const tx1 = STX + 3, tx2 = STX + STW - 3, tcx = STX + STW / 2;
                    return (
                      <g key={`${u.id}str`}>
                        {hasGN && (
                          // Normal: base ancha abajo, vértice arriba
                          <path d={`M${tx1},${topY+hpx} L${tx2},${topY+hpx} L${tcx},${topY} Z`}
                            fill="rgba(110,70,20,0.28)" stroke={T.ink} strokeWidth="0.7" />
                        )}
                        {hasGR && (
                          // Inversa: base ancha arriba, vértice abajo
                          <path d={`M${tcx},${topY+hpx} L${tx1},${topY} L${tx2},${topY} Z`}
                            fill="rgba(110,70,20,0.28)" stroke={T.ink} strokeWidth="0.7" />
                        )}
                        {shown.map((sid, j) => (
                          <g key={`${u.id}sym${j}`}
                ...
                {/* BI bars */}
                <g clipPath="url(#biclip)">
                  {units.map((u, i) => {
                    if (u.bioturbIndex == null) return null;
                    const topY = yOf(cumH[i] + u.thickness);
                    const hpx  = u.thickness * SPX;
                    const barW = (u.bioturbIndex / 6) * BTW;
                    const biColor = u.bioturbIndex <= 1 ? "#A8C8E8"
                      : u.bioturbIndex <= 3 ? "#6699BB"
                      : u.bioturbIndex <= 5 ? "#336699"
                      : "#1A4466";
                    return (
                      <g key={`${u.id}bi`}>
                        <rect x={BTX} y={topY} width={barW} height={Math.max(1, hpx)}
                          fill={biColor} fillOpacity="0.7" />
                        {hpx > 14 && (
                          <text x={BTX + BTW / 2} y={topY + hpx / 2 + 3.5}
                            textAnchor="middle" style={{ fontSize: 8,
                              fontFamily: MONO, fill: "#1A4466", fontWeight: 600 }}>
                            {u.bioturbIndex}
                          </text>
                        )}
                      </g>
                ...
                {/* Icnofósiles */}
                <g clipPath="url(#ichnoclip)">
                  {units.map((u, i) => {
                    const ichnos = (u.traceIchnos || []).filter(findI);
                    if (!ichnos.length) return null;
                    const topY = yOf(cumH[i] + u.thickness);
                    const hpx  = u.thickness * SPX;
                    const midY = topY + hpx / 2;
                    const gs   = 22;
                    const cx   = ITX + ITW / 2;
                    const maxN = Math.max(1, Math.floor(hpx / gs));
                    const shown = ichnos.slice(0, maxN);
                    const y0 = midY - (shown.length - 1) * gs / 2;
                    return (
                      <g key={`${u.id}icn`}>
                        {shown.map((iid, j) => (
                          <g key={`${u.id}icn${j}`}
                            transform={`translate(${cx},${y0 + j * gs}) scale(${gs / 26})`}>
                            {ichnoGlyphEls(iid, `${u.id}_i${j}`)}
                          </g>
                        ))}
                      </g>
                    );
                ...
                <g transform={`translate(${CX},${MT + totalH * SPX + 24})`}>
                  ...
                  {usedMods.length > 0 && (
                    <g transform={`translate(0,${modsTopY})`}>
                      <text x={0} y={0} style={{ fontSize: 12, fontFamily: SERIF,
                        fontStyle: "italic", fill: T.text, fontWeight: 600 }}>
                        Modificadores de zona</text>
                      <line x1={0} y1={4} x2={200} y2={4} stroke={T.gold} strokeWidth=".8" />
                      {usedMods.map((m, i) => {
                        const lx = (i % 2) * 270, ly = 18 + Math.floor(i / 2) * 17;
                        return (
                          <g key={m[0]} transform={`translate(${lx},${ly})`}>
                            <rect width="22" height="12" fill={`url(#${m[2]})`} fillOpacity="0.72"
                              stroke={T.ink} strokeWidth=".5" />
                            <text x={28} y={10} style={{ fontSize: 9.5, fontFamily: SANS,
                              fill: T.text2 }}>{m[1]}</text>
                          </g>
                        );
                      })}
                    </g>
                  ...
                  {usedIchno.length > 0 && (
                    <g transform={`translate(0,${ichnoTopY})`}>
                      <text x={0} y={0} style={{ fontSize: 12, fontFamily: SERIF,
                        fontStyle: "italic", fill: T.text, fontWeight: 600 }}>
                        Trazas fósiles / Icnofácies</text>
                      <line x1={0} y1={4} x2={200} y2={4} stroke={T.gold} strokeWidth=".8" />
                      {usedIchno.map((t, i) => {
                        const lx = (i % 2) * 270, ly = 18 + Math.floor(i / 2) * 17;
                        return (
                          <g key={t[0]} transform={`translate(${lx},${ly})`}>
                            <g transform="translate(10,5) scale(0.5)">{ichnoGlyphEls(t[0], `lg${t[0]}`)}</g>
                            <text x={28} y={9} style={{ fontSize: 9.5, fontFamily: SANS,
                              fill: T.text2 }}>{t[1]}</text>
                          </g>
                        );
                  ...
                  {hasBI && (
                    <g transform={`translate(0,${biTopY})`}>
                      <text x={0} y={0} style={{ fontSize: 12, fontFamily: SERIF,
                        fontStyle: "italic", fill: T.text, fontWeight: 600 }}>
                        Índice de bioturbación (BI)</text>
                      <line x1={0} y1={4} x2={200} y2={4} stroke={T.gold} strokeWidth=".8" />
                      {[0,1,2,3,4,5,6].map(v => {
                        const biColor = v <= 1 ? "#A8C8E8" : v <= 3 ? "#6699BB" : v <= 5 ? "#336699" : "#1A4466";
                        const desc = ["Sin bioturbación","<10 % bioturbado","10–40 %","40–60 %","60–90 %",">90 %","Bioturbación completa"][v];
                        return (
                          <g key={v} transform={`translate(${(v % 4) * 130},${18 + Math.floor(v / 4) * 16})`}>
                            <rect width="14" height="10" fill={biColor} fillOpacity="0.8"
                              stroke={T.ink} strokeWidth=".5" />
                            <text x={20} y={9} style={{ fontSize: 9, fontFamily: MONO,
                              fill: T.text2 }}><tspan fontWeight="700">{v}</tspan> {desc}</text>
                          </g>
                        );
                      })}
                    </g>
        ...
        <div onClick={() => setShowHelp(false)} style={{
          ...
          <div onClick={e => e.stopPropagation()} style={{
            ...
            <div style={{ marginTop: 16, padding: "12px 14px", background: T.panelAlt,
              ...
              · Distinguir <code>toba_lapilli</code> (piroclástica, fragmentos angulosos en matriz cinerítica) de <code>lapillite</code> (tufita, clastos redondeados retrabajados).<br/>
              ...
              · Aliases aceptados (CSV y .col.json viejos): <code>tuff</code>/<code>ash_tuff</code>→toba_ceniza, <code>lapilli_tuff</code>→toba_lapilli, <code>ignimbrite</code>→toba_ceniza, <code>volcanic_breccia</code>→brecha_pirocl.
            </div>
...
function MetaForm({ meta, setMeta, inputStyle, lblStyle, secStyle, btnBase, captureGps }) {
  ...
  const E2 = parseFloat(meta.utmE2), N2 = parseFloat(meta.utmN2);
  const secAz = [E1, N1, E2, N2].every(Number.isFinite) ? utmBearing(E1, N1, E2, N2) : null;
  return (
    <div style={{ padding: "12px 16px", display: "flex",
                  flexDirection: "column", gap: 9, flex: 1 }}>
      <div style={{ ...secStyle, marginTop: 0, paddingTop: 0,
      ...
      <div style={{ ...secStyle, color: T.green }}>Base / inicio (pie de la columna)</div>
      <div style={{ display: "flex", gap: 8 }}>
        <label style={{ ...lblStyle, flex: 1 }}>Este (m E)
          <input type="number" step="any" placeholder="345678"
            value={meta.utmE} onChange={e => m("utmE", e.target.value)} style={inputStyle} />
        </label>
        <label style={{ ...lblStyle, flex: 1 }}>Norte (m N)
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <label style={{ ...lblStyle, flex: 1 }}>Este (m E)
          <input type="number" step="any" placeholder="345820"
            value={meta.utmE2} onChange={e => m("utmE2", e.target.value)} style={inputStyle} />
        </label>
        <label style={{ ...lblStyle, flex: 1 }}>Norte (m N)
          <input type="number" step="any" placeholder="6123610"
      ...
      {secAz != null && (
        <div style={{ padding: "8px 10px", background: "#E3EEDD",
          border: `1px solid ${T.green}`, borderRadius: 3, fontSize: 11,
          color: T.text, fontFamily: MONO }}>
          ↳ Azimut de sección (base→techo): <b>{Math.round(secAz)}°</b>
          <div style={{ ...hint, marginTop: 2 }}>
            Dirección del recorrido (no el rumbo de los estratos). Orienta la
            vista al exportar a Google Earth.
          </div>
        </div>
      )}
      <div style={hint}>
        Chile continental: huso 18 o 19, hemisferio S, datum WGS84. Cargá la
        <b> base</b> para ubicar la columna; agregá el <b>techo</b> para calcular
        el azimut y dibujar la traza en <b>Google Earth (KMZ)</b>.
      </div>

      <div style={secStyle}>· Sección ·</div>
      <label style={lblStyle}>Azimut manual (grados; usado solo si no cargás el techo)
...
// ═══════════════════════════════════════════════════════════════
function SamplesForm({ samples, addSample, delSample, totalH, inputStyle, lblStyle, secStyle, btnPrimary, btnSmall, btnBase, requestGpsUtm, meta }) {
  const [h, setH] = useState("");
  const [code, setCode] = useState("");
  const [type, setType] = useState("geoquimica");
  const [utmE, setUtmE] = useState("");
  const [utmN, setUtmN] = useState("");
  const grabGps = () => requestGpsUtm && requestGpsUtm((u, coords) => {
    setUtmE(u.E.toFixed(1)); setUtmN(u.N.toFixed(1));
  });
  const submit = () => {
    const hv = parseFloat(h);
    if (!isFinite(hv) || !code.trim()) return;
    const s = { height: hv, code: code.trim(), type };
    if (utmE.trim() && utmN.trim()) { s.utmE = utmE.trim(); s.utmN = utmN.trim(); }
    addSample(s);
    setH(""); setCode(""); setUtmE(""); setUtmN("");
  };
  ...
  return (
    <div style={{ padding: "12px 16px", display: "flex", flexDirection: "column", gap: 9, flex: 1 }}>
      ...
      <div style={{ ...secStyle }}>· Coordenadas (opcional) ·</div>
      <div style={{ display: "flex", gap: 8 }}>
        <label style={{ ...lblStyle, flex: 1 }}>UTM Este
          <input type="number" step="any" placeholder="E" value={utmE}
            onChange={e => setUtmE(e.target.value)} style={inputStyle} />
        </label>
        <label style={{ ...lblStyle, flex: 1 }}>UTM Norte
          <input type="number" step="any" placeholder="N" value={utmN}
            onChange={e => setUtmN(e.target.value)} style={inputStyle} />
        </label>
      </div>
      <button onClick={grabGps} style={{ ...btnBase, fontSize: 10.5 }}>
        📍  Capturar GPS aquí</button>
      <div style={hint}>
        Toma la posición del punto de muestreo. Zona {meta?.utmZone || "—"}{meta?.utmHemi || "S"} ·
        solo en https/PWA. Dejá en blanco si no aplica.
      </div>
            ...
            return (
              <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 8,
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 9.5, color: T.text3, fontStyle: "italic" }}>
                    {t[1]}{s.utmE && s.utmN ? ` · 📍 ${Math.round(s.utmE)}E / ${Math.round(s.utmN)}N` : ""}
                  </div>
                </div>
